...
class ExampleBenchmark {
    ...
    private fun MacrobenchmarkScope.scrollList() {
        device.wait(Until.hasObject(By.res("news_list")), 3000)
    ...
    private fun MacrobenchmarkScope.clickBookmark() {
        device.wait(Until.hasObject(By.res("bookmark_icon")), 3000)
        val bookmark = device.findObject(By.res("bookmark_icon"))
        bookmark.click()
        device.waitForIdle()
    }

    private fun MacrobenchmarkScope.goBookmarkScreen() {
        device.wait(Until.hasObject(By.text("Bookmark")), 3000)
        val bookmark = device.findObject(By.text("Bookmark"))
        bookmark.click()
        device.waitForIdle()
    }
}